...
from datasets.pdbbind import construct_loader, construct_protein_loader
...
def train(args, model, optimizer, scheduler, ema_weights, train_loader, val_loader, t_to_sigma, run_dir):
    ...
    for epoch in range(args.n_epochs):
        ...
        print("Epoch {}: Training loss {:.4f}       side {:.4f}       side_base {:.4f}"
              .format(epoch, train_losses['loss'],
              train_losses['side_loss'],
            train_losses['side_base_loss']))
        ...
        print("Epoch {}: Validation loss {:.4f}       side {:.4f}       side_base {:.4f}"
              .format(epoch, val_losses['loss'],
                val_losses['side_loss'], 
                val_losses['side_base_loss']))
        ...
        if args.val_inference_freq != None and (epoch) % args.val_inference_freq == 0:
            ...
            print("Epoch {}: Val inference rmsds {:.3f}       rmsds_lt1 {:.3f}       rmsds_lt1.25 {:.3f}"
                  .format(epoch, inf_metrics['rmsd'], inf_metrics['rmsds_lt1'], inf_metrics['rmsds_lt1.25']))
...
def main_function():
    ...
    train_loader, val_loader = construct_protein_loader(args, t_to_sigma)

    model = get_model(args, device, t_to_sigma=t_to_sigma, only_protein= True)
