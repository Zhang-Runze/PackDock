...
from datasets.process_mols import parse_pdb_from_path, generate_conformer, read_molecule, get_lig_graph_with_matching, \
    extract_receptor_structure, get_rec_graph, parse_receptor
...
class InferenceDataset(Dataset):
    ...
    def get(self, idx):
        ...
        name, protein_file, ligand_description, all_lm_embedding = \
            self.complex_names[idx], self.protein_files, self.ligand_descriptions, self.lm_embeddings

        LM_embedding = all_lm_embedding[name]
        ...
        try:
            mol = read_mols(pdbbind_dir, name, remove_hs=False)
        except Exception as e:
            print('Failed to read molecule ', name, ' We are skipping it. The reason is the exception: ', e)
            complex_graph['success'] = False
            return complex_graph
        try:
            rec_model = parse_receptor(name, pdbbind_dir)
            mol = mol[0]
            ...
            rec, rec_coords, c_alpha_coords, Rotamer_0, Rotamer_1, Rotamer_2, Rotamer_3, physicochemistry, PSP19, n_coords, c_coords, lm_embeddings = extract_receptor_structure(rec_model, mol, lm_embedding_chains=LM_embedding)
            if lm_embeddings is not None and len(c_alpha_coords) != len(lm_embeddings):
                print(f'LM embeddings for complex {name} did not have the right length for the protein. Skipping {name}.')
                complex_graph['success'] = False
                return complex_graph
            get_rec_graph(rec, rec_coords, c_alpha_coords, Rotamer_0, Rotamer_1, Rotamer_2, Rotamer_3, physicochemistry, PSP19, n_coords, c_coords, complex_graph, rec_radius=self.receptor_radius,
        ...
        rotamer_0 = complex_graph['receptor'].rotamer_0
        rotamer_1 = complex_graph['receptor'].rotamer_1
        rotamer_2 = complex_graph['receptor'].rotamer_2
        rotamer_3 = complex_graph['receptor'].rotamer_3

        complex_graph.original_rotamer_0 = rotamer_0
        complex_graph.original_rotamer_1 = rotamer_1
        complex_graph.original_rotamer_2 = rotamer_2
        complex_graph.original_rotamer_3 = rotamer_3
