...
from utils.utils import read_strings_from_txt, get_pocket_outside_num, get_ligand_outside_num
...
class NoiseTransform(BaseTransform):
    ...
    def apply_noise(self, data, t, side_updates=None):
        if not torch.is_tensor(data['ligand'].pos):   ###when only traing protein score model
            data['ligand'].pos = random.choice(data['ligand'].pos)

        side_sigma = self.t_to_sigma(t)
        set_time(data, t, 1, self.all_atom, device=None)
        side_updates = np.random.normal(loc=0.0, scale=side_sigma, size=(data.tor_mask.sum()))
        modify_conformer(data, side_updates)
        data.side_sigma = np.ones(data.tor_mask.sum()) * side_sigma

        data.side_score = torch.from_numpy(torus.score(side_updates, side_sigma)).float()
...
class PDBBind(Dataset):
    ...
    def preprocessing(self):
        ...
        if self.esm_embeddings_path is not None:
            dictlist = open(self.esm_embeddings_path, 'rb')
            chain_embeddings_dictlist = pickle.load(dictlist)
        ...
        if self.num_workers > 1:
            ...
            for i in range(len(complex_names_all)//1000+1):
                ...
                if self.num_workers > 1:
                    p = Pool(maxtasksperchild=6,processes = self.num_workers)
                    p.__enter__()
    ...
    def get_complex(self, par):
        ...
        if ligand is not None:
            ...
            try:
                rec_model,pdb_mol = parse_receptor(name, self.pdbbind_dir)
            except Exception as e:
        ...
        for i, lig in enumerate(ligs):
            ...
            try:
                ...
                get_rec_graph(rec, pdb_mol, rec_coords, c_alpha_coords, rotamer_idx, backbone_idx, physicochemistry, PSP19, n_coords, c_coords, ligand_outside_num, ligand_outside_idx, complex_graph, rec_radius=self.receptor_radius,
...
class PDBprotein(Dataset):
    def __init__(self, root, transform=None, cache_path='data/cache', split_path='data/', limit_complexes=0,
        ...
        super(PDBprotein, self).__init__(root, transform)
        self.pdbbind_dir = root
        self.max_lig_size = max_lig_size
        self.split_path = split_path
        self.limit_complexes = limit_complexes
    ...
    def preprocessing(self):
        print(f'Processing proteins from [{self.split_path}] and saving it to [{self.full_cache_path}]')

        complex_names_all = read_strings_from_txt(self.split_path)
        if self.limit_complexes is not None and self.limit_complexes != 0:
            complex_names_all = complex_names_all[:self.limit_complexes]
        print(f'Loading {len(complex_names_all)} proteins.')
...
def construct_loader(args, t_to_sigma):
    ...
    loader_class = DataListLoader if torch.cuda.is_available() else DataLoader   #parallel
...
def construct_protein_loader(args, t_to_sigma):
    transform = NoiseTransform(t_to_sigma=t_to_sigma, no_torsion=args.no_torsion,
                               all_atom=args.all_atoms)
    ...
    train_dataset = PDBprotein(cache_path=args.cache_path, split_path=args.split_train, keep_original=True,
                            num_conformers=args.num_conformers, **common_args)
    val_dataset = PDBprotein(cache_path=args.cache_path, split_path=args.split_val, keep_original=True, **common_args)
...
def read_pdb(pdbbind_dir, name, remove_hs=False):
    path = os.path.join(pdbbind_dir, name, f'{name}_protein.pdb')
    mol = Chem.MolFromPDBFile(path, sanitize=True, removeHs=True)
    return mol

def read_protein_pdb(pdbbind_dir, name, remove_hs=False):
    path = os.path.join(pdbbind_dir, name)
    mol = Chem.MolFromPDBFile(path, sanitize=True, removeHs=True)
    return mol
