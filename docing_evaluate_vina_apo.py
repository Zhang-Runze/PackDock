...
from utils.utils import gpu_docking_with_ligand_boxsize_sdf, get_model, extract_protein_packing_state, pdb2pdbqt, read_first_docking_log, get_pocket_outside_num, get_ligand_outside_num
...
def extract_protein_docking_top(docking_dataframe_list, PDBID, out_path):
    ...
    sorted_df = merged_df.sort_values('affinity')
    for i in range(1,7):
        try:
    ...
    if not os.path.exists(output_dataframe):
        ...
        while attempt <= max_attempts:
            ...
            try:
                ...
                pocket_packing_start_time = time.time()
                protein_graphs = construct_protein_graph(ori_protein_path)
                packing_result_path = protein_side_chain_packing(protein_graphs)
                pocket_packing_end_time = time.time()

                full_packing_result = []
                for packing_protein_path in packing_result_path:
                    full_packing_result.append(extract_protein_packing_state(packing_protein_path))

                num_clusters = 6
                packing_result_path = cluster_proteins(full_packing_result, num_clusters)

                protein_docking_dataframes = []
                loop_num = 0 
                for idx, packing_protein_path in enumerate(packing_result_path):

                    copy_mol2_file = copy_file(ori_LIG_pdb_file, out_path)
                    protein_path = extract_protein_packing_state(packing_protein_path)
                    protein_path, ligand_path = pdb2pdbqt(protein_path, copy_mol2_file, remove_protein=True, remove_ligand=True)

                    ligand_states_path ,pdbqt_protein_path = gpu_docking_with_ligand_boxsize_sdf(protein_path, ligand_path, SDF_file, out_path, loop_num, PDBID, args.seed, idx)

                    os.chdir(os.path.dirname(__file__))
                    protein_log_path = f"{out_path}/{PDBID}_0_{idx}_docking_ligand_vina_score_out.log"

                    if args.rmsd:
                        protein_docking_df = read_first_docking_log(protein_log_path)
                        protein_docking_rmsd = []
                        nums = []
                        for ligand_state_path in ligand_states_path:
                            rmsd_txt = f"{out_path}/rmsd.txt"
                            os.system(f'obrms {ligand_path} {ligand_state_path} > {rmsd_txt}')
                ...
                for idx, packing_protein_path in enumerate(packing_result_path):
                    ...
                    os.chdir(os.path.dirname(__file__))
                    protein_log_path = f"{out_path}/{PDBID}_1_{idx}_docking_ligand_vina_score_out.log"
                    if args.rmsd:
                        protein_docking_df = read_first_docking_log(protein_log_path)
                        protein_docking_rmsd = []
                        nums = []
                        for ligand_state_path in ligand_states_path:
                            rmsd_txt = f"{out_path}/rmsd.txt"
                            os.system(f'obrms {ori_LIG_pdb_file} {ligand_state_path} > {rmsd_txt}')
                            rmsd = extract_rmsd_from_txt(rmsd_txt)
                            protein_docking_rmsd.append(rmsd)
                            nums.append(idx)
                        protein_docking_df = protein_docking_df.head(len(protein_docking_rmsd))
                ...
                ligand_packing_elapsed_time = ligand_packing_end_time - ligand_packing_start_time
                pocket_packing_elapsed_time = pocket_packing_end_time - pocket_packing_start_time
                time_info = {
                    "elapsed_time": elapsed_time,
                    "ligand_packing_elapsed_time": ligand_packing_elapsed_time,
                    "pocket_packing_elapsed_time": pocket_packing_elapsed_time
                }

                elapsed_time_txt = f"{out_path}/1_elapsed_time.txt"

                with open(elapsed_time_txt, "w") as file:
                    for key, value in time_info.items():
                        file.write(f"{key}: {value}\n")
                print(f"code elapsed_time: {elapsed_time} seconds")
                break
